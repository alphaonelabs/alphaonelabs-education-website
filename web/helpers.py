...
def create_subscription(user, plan_id, payment_method_id, billing_period):
    ...
    try:
        ...
        if not price_id:
            return {
                'success': False,
                'message': f'The {billing_period} billing period is not available for this plan'
            }
        ...
        return {
            'success': False,
            'message': 'An error occurred with the payment processor. Please try again.'
        }
    except Exception as e:
    ...
    try:
        ...
        if hasattr(subscription, 'items') and subscription.items.data:
            ...
            try:
                ...
                ).first() or MembershipPlan.objects.filter(
