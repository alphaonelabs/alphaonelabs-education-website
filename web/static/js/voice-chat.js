...
class EncryptedVoiceChat {
  ...
  async initializeWebSocket() {
    ...
    const connect = () => {
      ...
      this.socket = new WebSocket(
        `${protocol}://${window.location.host}/ws/voice-chat/${formattedUuid}/`
      );
      ...
      this.socket.onmessage = async (event) => {
        try {
          ...
          console.log(
            `[RECV] ${data.type} from ${data.from || "server"} to ${
              data.target || "all"
            }`
          );
      ...
      this.socket.onclose = (event) => {
        ...
        if (
          ...
          alert(
            "Unable to connect to voice chat. Please refresh the page to try again."
          );
        }
      };
  ...
  formatUUID(uuid) {
    ...
    try {
      ...
      if (/^[0-9a-f]{32}$/i.test(cleaned)) {
        return `${cleaned.substring(0, 8)}-${cleaned.substring(
          8,
          12
        )}-${cleaned.substring(12, 16)}-${cleaned.substring(
          16,
          20
  ...
  updateParticipantsList(participants) {
    ...
    participants.forEach(async (participant) => {
      if (participant.id !== this.userId) {
        ...
        const speakingIndicator = document.getElementById(
          `speaking-indicator-${participant.id}`
        );
        if (speakingIndicator) {
          speakingIndicator.style.display = participant.is_speaking
            ? "block"
            : "none";
        }

        const mutedIndicator = document.getElementById(
          `muted-indicator-${participant.id}`
        );
        if (mutedIndicator) {
          mutedIndicator.style.display = participant.is_muted
            ? "block"
            : "none";
        }
  ...
  async handleUserJoined(userId, username) {
    ...
    if (this.encryptionKeys[this.userId]) {
      try {
        const keyData = await this.exportPublicKeyData(
          this.encryptionKeys[this.userId]
        );
      ...
      console.warn(
        `Could not share encryption key with ${userId} as it is not available`
      );
    }
  ...
  async handleOffer(data) {
    ...
    try {
      console.log(
        `Received offer from ${from}, state: ${
          this.peers[from]?.signalingState || "no connection"
        }`
      );
      ...
      if (signalingState !== "stable") {
        console.warn(
          `Cannot set remote offer in state: ${signalingState}, rolling back`
        );
      ...
      console.log(
        `Set remote offer from ${from}, new state: ${pc.signalingState}`
      );
  ...
  async handleAnswer(data) {
    ...
    try {
      ...
      const signalingState = pc.signalingState;
      console.log(
        `Received answer from ${from}, current state: ${signalingState}, hasPendingOffer: ${pc.hasPendingOffer}`
      );

      // Only process the answer if we sent an offer
      if (!pc.hasPendingOffer) {
        console.warn(
          `Received answer from ${from} but we didn't send an offer`
        );
        return;
      }
      ...
      if (signalingState === "have-local-offer") {
        ...
        console.log(
          `Successfully set remote answer from ${from}, new state: ${pc.signalingState}`
        );
        // Clear the pending offer flag
        pc.hasPendingOffer = false;
      } else if (signalingState === "stable") {
        console.log(
          `Connection with ${from} already established, ignoring redundant answer`
        );
        pc.hasPendingOffer = false;
      } else {
        console.warn(
  ...
  async handleIceCandidate(data) {
    ...
    try {
      const pc = this.peers[from];
      console.log(
        `Adding ICE candidate from ${from}, state: ${pc.signalingState}`
      );
      await pc.addIceCandidate(new RTCIceCandidate(candidate));
    } catch (error) {
      // It's normal for this to throw errors if the remote description isn't set yet
      console.warn(
        `Failed to add ICE candidate from ${from}: ${error.message}`
      );
    }
  }
  ...
  createParticipantElement(userId, displayName, isCurrentUser) {
    ...
    const baseClasses =
      "bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 w-28 flex flex-col items-center";
    element.className = isCurrentUser
      ? `${baseClasses} ring-2 ring-teal-500 dark:ring-teal-400`
      : baseClasses;
  ...
  handleRemoteTrack(userId, stream) {
    ...
    if (!audioElement) {
      ...
      document
        .getElementById("remote-audio-container")
    ...
    if (this.audioContext.audioWorklet) {
      ...
      const detectVoiceActivity = (rms) => {
        ...
        if (isSpeaking !== this.speaking && !this.muted) {
          ...
          const speakingIndicator = document.getElementById(
            `speaking-indicator-${this.userId}`
          );
          if (speakingIndicator) {
      ...
      console.warn(
        "Using deprecated ScriptProcessorNode. Consider upgrading your browser."
      );
  ...
  createRemoteVisualization(userId, displayName) {
    ...
    visualizerContainer.className =
      "bg-gray-100 dark:bg-gray-700 p-3 rounded-lg mb-3";
  ...
  createAudioVisualization(userId, stream) {
    ...
    const visualize = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        const mutedIndicator = document.getElementById(
          `muted-indicator-${userId}`
        );
        const isMuted =
          mutedIndicator && mutedIndicator.style.display === "block";
  ...
  sendSignalingMessage(message, retryCount = 0) {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      console.warn(
        `Cannot send message of type ${message.type}: WebSocket not open`
      );
      ...
      if (retryCount < this.maxRetries) {
        ...
        console.error(
          `Failed to send message type ${message.type} after ${this.maxRetries} attempts`
        );
      }
      return;
    }
    ...
    try {
      console.log(
        `[SEND] ${message.type} to ${message.target || "all"} from ${
          this.userId
        }`
      );
