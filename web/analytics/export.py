...
from web.models import (
    ...
    SessionAttendance, Achievement, User
)
from .utils import (
...
def export_pdf(request, analytics_type, obj_id=None):
    ...
    if analytics_type == 'course':
        ...
        # Add title
        elements.append(Paragraph(f"Student Analytics Report: {student.get_full_name() or student.username}", title_style))
        elements.append(Spacer(1, 0.25*inch))
        elements.append(Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", normal_style))
        elements.append(Spacer(1, 0.5*inch))
        ...
        if enrollments:
            ...
            attendance_table = Table(attendance_data, colWidths=[1.5*inch, 0.75*inch, 0.75*inch, 0.75*inch, 0.75*inch, 1*inch])
