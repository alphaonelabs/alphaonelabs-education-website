...
class MassClassStream(models.Model):
    ...
    stream_id = models.UUIDField(unique=True)
    teacher = models.ForeignKey(User, on_delete=models.CASCADE, related_name="mass_class_streams")
    course = models.ForeignKey(
        Course, on_delete=models.SET_NULL, null=True, blank=True, related_name="mass_class_streams"
    )
    session = models.ForeignKey(
        Session, on_delete=models.SET_NULL, null=True, blank=True, related_name="mass_class_streams"
    )
...
class MassClassViewer(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} viewing {self.stream.session_id}"
...
class MassClassPoll(models.Model):
    ...
    poll_id = models.UUIDField(unique=True)
...
class MassClassHandRaise(models.Model):
    ...
    def __str__(self):
        return f"Hand raise by {self.user.username}"
