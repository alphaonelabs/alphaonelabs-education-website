...
def join_session_waiting_room(request, course_slug):
    ...
    session_waiting_room, created = SessionWaitingRoom.objects.get_or_create(
        course=course,
    ...
    if request.user not in session_waiting_room.participants.all():
        ...
        if next_session:
            ...
            messages.success(request, f"You have joined the waiting room for the next session of {course.title}. You'll be notified when a new session is scheduled.")
    else:
