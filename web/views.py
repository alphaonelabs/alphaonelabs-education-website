...
def add_chapter_resource(request, slug):
    """Add a new resurce to the chapter."""
    chapter = get_object_or_404(Chapter, slug=slug)
    membership = get_object_or_404(ChapterMembership, chapter=chapter, user=request.user, is_approved=True)

    # Check if the user has permission to add resources
    if not membership.is_approved:
        messages.error(request, "You must be an approved member of this chapter to add resources.")
        return redirect("chapter_detail", slug=slug)
    membership = get_object_or_404(ChapterMembership, chapter=chapter, user=request.user, is_approved=True)
...
def edit_chapter_resource(request, slug, resource_id):
    ...
    # Only allow resource creator, leaders and co-organizers to edit
    if membership.role not in ["lead", "co_organizer"] and resource.created_by != request.user:
...
def delete_chapter_resource(request, slug, resource_id):
    ...
    # Only allow resource creator, leaders and co-organizers to delete
    if membership.role not in ["lead", "co_organizer"] and resource.created_by != request.user:
    ...
    if request.method == "POST":
        resource.delete()
        messages.success(request, "Resource has been deleted successfully.")
        return redirect("chapter_detail", slug=slug)
