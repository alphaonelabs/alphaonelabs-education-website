    ...
    def setUp(self):
        ...
        self.canceled_membership = UserMembership.objects.create(
            user=User.objects.create_user(
                username="canceleduser", email="canceled@example.com", password="testpass123"
            ),
            plan=self.plan,
            status="active",
            billing_period="monthly",
...
class MembershipSubscriptionEventModelTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="eventuser", email="event@example.com", password="testpass123")
    ...
    def test_ordering(self):
        ...
        MembershipSubscriptionEvent.objects.create(
            user=self.user, membership=self.membership, event_type="updated", stripe_event_id="evt_newer123"
        )

        # Get all events for this user
        events = MembershipSubscriptionEvent.objects.filter(user=self.user).values_list("event_type", flat=True)
