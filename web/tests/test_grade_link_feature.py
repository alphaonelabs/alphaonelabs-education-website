...
class GradeableLinkModelTest(TestCase):
    ...
    def test_average_grade_with_grades(self):
        ...
        LinkGrade.objects.create(
            ...
            user=user3,
            grade='B+',
            comment='Good job, but could be improved.'
        )
...
class LinkGradeModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.grade = LinkGrade.objects.create(
            ...
            user=self.grader,
            grade='A-',
            comment='Good work, but could use some improvements.'
        )
    ...
    def test_unique_together_constraint(self):
        ...
        with self.assertRaises(Exception):  # Could be IntegrityError or ValidationError depending on Django version
            LinkGrade.objects.create(
                link=self.link,
                user=self.grader,  # Same user as self.grade
                grade='B',
...
class GradeableLinkViewsTest(TestCase):
    ...
    def test_link_create_view_unauthenticated(self):
        ...
        self.assertRedirects(
            response, 
            f"{reverse('account_login')}?next={reverse('gradeable_link_create')}"
        )
    ...
    def test_link_create_submission(self):
        ...
        response = self.client.post(reverse('gradeable_link_create'), {
            'title': 'New Test Link',
    ...
    def test_grade_link_view_unauthenticated(self):
        ...
        self.assertRedirects(
            response, 
            f"{reverse('account_login')}?next={reverse('grade_link', args=[self.link.pk])}"
        )
    ...
    def test_grade_link_submission(self):
        ...
        response = self.client.post(reverse('grade_link', args=[self.link.pk]), {
            'grade': 'B+',
            'comment': 'Good job, but could be improved.'
        })
        ...
        self.assertTrue(LinkGrade.objects.filter(
            link=self.link,
            user=self.grader,
            grade='B+',
            comment='Good job, but could be improved.'
    ...
    def test_grade_link_submission_invalid(self):
        ...
        response = self.client.post(reverse('grade_link', args=[self.link.pk]), {
            'grade': 'B+',
            'comment': ''  # No comment, should be invalid for B+
        })
        ...
        self.assertFalse(LinkGrade.objects.filter(
            link=self.link,
            user=self.grader
        ).exists())
    ...
    def test_update_existing_grade(self):
        ...
        grade = LinkGrade.objects.create(
            link=self.link,
            user=self.grader,
            grade='B',
