...
class QuizViewTests(TestCase):
    def setUp(self):
        ...
        self.question = QuizQuestion.objects.create(quiz=self.quiz, question_text="What is the capital of France?")

        # Create options for the question
        self.correct_option = QuizOption.objects.create(question=self.question, option_text="Paris", is_correct=True)
    ...
    def test_current_live_quiz_view_already_submitted(self):
        ...
        QuizSubmission.objects.create(user=self.user, quiz=self.quiz, score=80)
    ...
    def test_submit_quiz_without_login(self):
        ...
        response = self.client.post(url, {f"question_{self.question.id}": self.correct_option.id})
    ...
    def test_submit_quiz_with_incorrect_answer(self):
        ...
        url = reverse("submit_quiz", args=[self.quiz.id])
        response = self.client.post(url, {f"question_{self.question.id}": self.incorrect_option.id})
