...
def label_issue_by_date(owner, repo, issue_number, created_at, headers):
    ...
    try:
        ...
        created_date = datetime.strptime(created_at.replace("Z", "+00:00").split("+")[0], "%Y-%m-%dT%H:%M:%S")
    ...
    # Check if any other date label exists (format: YYYY-MM)
    existing_date_labels = [label for label in current_labels if len(label) == 7 and label[4] == "-"]
