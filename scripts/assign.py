    ...
    if event_name == "issue_comment" and issue and comment:
        ...
        if is_assign:
            ...
            try:
                ...
                # Get issue details for further checks
                print(f"Fetching issue details from {issue_url}")
                issue_response = requests.get(issue_url, headers=headers)
                print(f"Issue details response status: {issue_response.status_code}")
                issue_data = issue_response.json()
                ...
                if has_good_first_issue_label:
                    print(f"Issue #{issue_number} is labeled as 'good first issue'. Checking merged PRs for user {user_login}")
                    ...
                    print(f"Merged PR search response status: {search_response.status_code}")
                    if search_response.status_code != 200:
                        print(f"Error during merged PR search: {search_response.status_code}")
                    else:
                        search_data = search_response.json()
                        merged_pr_count = search_data.get("total_count", 0)
                        print(f"User {user_login} has {merged_pr_count} merged PR(s)")
                        if merged_pr_count > 0:
                            comment_body = (
                                f"@{user_login} This issue is labeled as 'good first issue' and is reserved for new contributors. "
                                f"You already have {merged_pr_count} merged pull request(s). Please choose a different issue."
                            )
                            print(f"Rejecting assignment for user {user_login} due to existing merged PRs.")
