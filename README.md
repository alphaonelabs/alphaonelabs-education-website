...
The project includes a GitHub Actions workflow for automated production deployment. This workflow:

1. **Creates a release** based on semantic versioning (major, minor, or patch)
2. **SSH to the production server** using credentials from GitHub secrets
3. **Pulls latest code** from the main branch
4. **Installs dependencies** using Poetry
5. **Runs database migrations**
6. **Collects static files**
7. **Restarts the web server** (Gunicorn/Uvicorn and Nginx)

#### Setting Up GitHub Secrets

Before running the deployment workflow, configure the following secrets in your GitHub repository (Settings → Secrets and variables → Actions):

- `PRODUCTION_SERVER_IP`: The IP address of your production server
- `PRODUCTION_SERVER_USER`: SSH username (e.g., `django` or `root`)
- `PRODUCTION_SERVER_PASSWORD`: SSH password for the user
...
For manual deployment using Ansible:
...
Ensure you have configured `ansible/inventory.yml` with your server details.
